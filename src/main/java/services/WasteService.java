...
public class WasteService {
    ...
    public void addWaste(String type, double weight, String status, String createdAt) {
        if (model == null) {
            loadRDF();
        }


        String waste_id = UUID.randomUUID().toString();
    ...
    public String getWasteById(String wasteId) {
        if (model == null) {
            loadRDF();
        }
        ...
        if (model.containsResource(wasteResource)) {
            JSONObject wasteJson = new JSONObject();


            StmtIterator properties = wasteResource.listProperties();
            while (properties.hasNext()) {
                ...
                wasteJson.put(property, value);
            }


            return wasteJson.toString();
        } else {

            return "{\"error\": \"Déchet non trouvé: " + wasteId + "\"}";
        }
    }
    ...
    public String queryWastesByStatus(String status) {
        loadRDF();
        ...
        String queryString =
                "PREFIX ontologie-dechet: <http://www.semanticweb.org/user/ontologies/2024/8/untitled-ontology-5#> " +
                        ...
                        "WHERE { " +
                        "  ?waste a ontologie-dechet:Déchet . " +
                        "  ?waste ontologie-dechet:status \"" + status + "\" . " +
                        "  ?waste ?property ?value . " +
                        "}";
